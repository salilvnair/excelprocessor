...
import com.github.salilvnair.excelprocessor.v2.test.archived.sheet.dynamic.task.DynamicSheetTask;
import com.github.salilvnair.excelprocessor.v2.test.archived.sheet.dynamic.task.DynamicSheetValidatorTask;
...
//        read();
    ...
    public static List<? extends BaseSheet> read() throws Exception {
        ...
        String filePath = "C:\\Users\\sn2527\\workspace\\experiments\\excelprocessor\\src\\main\\resources\\excel\\DynamicallyGeneratedExcelProcessorTest.xlsx";
...
//        excelSheetContext.setDynamicHeaderDisplayNames(dynamicHeaderDisplayNames);
    ...
    private static void write1() throws Exception {
        String filePath = "C:\\Users\\sn2527\\workspace\\experiments";
        ExcelSheetWriter writer = ExcelSheetWriterFactory.generate();
        ExcelSheetContext excelSheetContext = ExcelSheetContext
                                                    .builder()
                                                    .filePath(filePath)
                                                    .fileName("Horizontal_Dynamic_Sheet_Test.xlsx")
                                                    .build();
        Workbook workbook = writer.workbook(prepareDynamicSchoolSheet1(), excelSheetContext);
        ExcelSheetWriterUtil.write(workbook, excelSheetContext.fileName(), excelSheetContext.filePath());
    }

    private static List<DynamicSchoolSheet> prepareDynamicSchoolSheet1() {
        List<DynamicSchoolSheet> dynamicSchoolSheets = new ArrayList<>();
        DynamicSchoolSheet dynamicSchoolSheet = new DynamicSchoolSheet();
        LinkedHashMap<String, CellInfo> dynamicHeaderKeyedCellInfoMap = new LinkedHashMap<>();
        CellInfo cellInfo = CellInfo.builder().header("FirstName").value("Salil").build();
        dynamicHeaderKeyedCellInfoMap.put("FirstName", cellInfo);
        cellInfo = CellInfo.builder().header("LastName").value("Nair").build();
        dynamicHeaderKeyedCellInfoMap.put("LastName", cellInfo);
        dynamicSchoolSheet.setDynamicHeaderKeyedCellInfoMap(dynamicHeaderKeyedCellInfoMap);
        dynamicSchoolSheets.add(dynamicSchoolSheet);


        dynamicSchoolSheet = new DynamicSchoolSheet();
        dynamicHeaderKeyedCellInfoMap = new LinkedHashMap<>();
        cellInfo = CellInfo.builder().header("FirstName").value("Reshmi").build();
        dynamicHeaderKeyedCellInfoMap.put("FirstName", cellInfo);
        cellInfo = CellInfo.builder().header("LastName").value("Pillai").build();
        dynamicHeaderKeyedCellInfoMap.put("LastName", cellInfo);
        dynamicSchoolSheet.setDynamicHeaderKeyedCellInfoMap(dynamicHeaderKeyedCellInfoMap);
        dynamicSchoolSheets.add(dynamicSchoolSheet);
        return dynamicSchoolSheets;
