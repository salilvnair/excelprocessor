...
import com.github.salilvnair.excelprocessor.v2.test.sheet.vertical.notemplate.CompanyInfoVerticalDynamicSheet;
import com.github.salilvnair.excelprocessor.v2.test.sheet.vertical.notemplate.CompanyInfoVerticalSheet;

import java.util.LinkedHashMap;
import java.util.List;

public class CompanyInfoNoTemplateVerticalSheetMock {
    ...
    private static List<CompanyInfoVerticalDynamicSheet> companyInfoDynamicHorizontalSheets(List<CompanyInfoMock.CompanyInfo> companyInfos) {
       return companyInfos.stream()
               .map(CompanyInfoNoTemplateVerticalSheetMock::generateHorizontalDynamicHeaders)
               .toList();
   }
    ...
    private static List<CompanyInfoVerticalSheet> companyInfoHorizontalSheets(List<CompanyInfoMock.CompanyInfo> companyInfos) {
        return companyInfos.stream()
                .map(CompanyInfoNoTemplateVerticalSheetMock::createCompanyInfoHorizontalSheet)
                .toList();
    }
